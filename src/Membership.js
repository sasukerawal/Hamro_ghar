...
import { ListingModal, formatPrice } from "./ListingUtils";

export default function Membership({
  onLogout,
  onGoHome,
  onChatWithUser, // ✅ new chat handler from App
}) {
  ...
  const handleEditListingSafe = (id) => {
    if (!id) return;
    // We navigate directly here; no need for onEditListing prop
    ...
    const check = async () => {
      try {
        ...
        if (e.message.includes("401") || e.message.includes("Unauthorized")) {
          // Not a member / not logged in → send home
  ...
  return (
    <div className="min-h-[calc(100vh-5rem)] bg-slate-50 flex items-center justify-center px-4 py-10">
      ...
      {isModalOpen && (
        <ListingModal
          // Using the **merged** props style that your current ListingModal supports
          isOpen={isModalOpen}
          onClose={closeHomeModal}
          home={selectedHome}
          context={modalContext} // "saved" or "owned"
          onUnsave={onUnsaveHandler}
          onEditListing={handleEditListingSafe}
          onChat={onChatWithUser} // ✅ chat handler
        />
      )}
    </div>
  );
